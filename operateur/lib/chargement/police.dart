...
class Polices extends StatefulWidget {
  const Polices({Key? key}) : super(key: key);
...
class _PolicesState extends State<Polices> {
  ...
  Future<void> fetchData() async {
    try {
      final response = await http.get(
        Uri.parse("http://192.168.43.149:81/projetSV/police.php"),
      );
  ...
  Future<void> fetchRecordCount() async {
    try {
      final response = await http.get(
        Uri.parse(
            "http://192.168.43.149:81/projetSV/recordstate.php?table_names=police&services=police"),
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: const Center(
          child: Text('Signales des détresses Polices'),
        ),
