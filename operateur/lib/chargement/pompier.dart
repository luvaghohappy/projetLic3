...
class Pompier extends StatefulWidget {
  const Pompier({Key? key}) : super(key: key);

  @override
  State<Pompier> createState() => _PompierState();
}

class _PompierState extends State<Pompier> {
  ...
  Future<void> fetchData() async {
    try {
      final response = await http.get(
        Uri.parse("http://192.168.43.149:81/projetSV/pompier.php"),
      );
  ...
  Future<void> fetchRecordCount() async {
    try {
      final response = await http.get(
        Uri.parse(
            "http://192.168.43.149:81/projetSV/recordstate.php?table_names=pompier&services=pompier"),
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: const Center(
          child: Text('Signales des détresses Pompier'),
        ),
        leading: PopupMenuButton<String>(
          onSelected: (String result) {
            print('Selected: $result'); // Debugging
            switch (result) {
              case 'Utilisateurs':
                Navigator.push(
                  context,
